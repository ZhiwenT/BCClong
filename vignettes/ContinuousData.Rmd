...
```{r, echo=F, fig.cap = "Figure 1. Mean adjusted adherence for different numbers of clusters"}
knitr::include_graphics("../inst/extdata/MeanAdj.png")
```

## Fit BCC Model Using BCC.multi Function

Here, We used gaussian distribution for all three markers. The number of clusters was set to 2 because it has highest mean adjusted adherence. For the hyper-parameters of the adherence parameters $\alpha_r$, we set $\delta_{1,r} = \delta_{2,r} = 1$ for $r = 1,2,3$ For the global clustering proportion $\pi$, we set $\varphi = (1,...,1)$ which reflects no priori information favouring one cluster over the other. For the fixed effect coefficients $\gamma_{k,r}$, we are setting the value to list(1,1,1). For random effect $\theta$, we are setting the value to list(1,1,1).

We ran the model with 2,000 iterations, discard the first 1,000 sample, and kept every sample. This resulted in 1,000 samples for each model parameter. The MCMC sampling process took about 15 minutes on an AMD Ryzen$^{TM}$ 5 5600X desktop computer.
...
```{r, fig.height=10, fig.width=8, warning=F, message=F, fig.cap = "Figure 2. Local and global clustering"}
...
gp1 <- ggplot(data = dat[[1]], aes(x =time2, y =y, color=cluster.local,linetype=cluster.local,fill=cluster.local))+
  labs(caption = "Local clustering for Anxiety score")+
...
gp2 <- ggplot(data = dat[[2]], aes(x =time2, y =y, color=cluster.local,linetype=cluster.local,fill=cluster.local))+
  labs(caption = "Local clustering for Depress score")+
  geom_smooth(method = "loess", size = 3,se = FALSE,span=2)+
  theme_bw() + 
...
gp3 <- ggplot(data = dat[[3]], aes(x =time2, y =y, color=cluster.local,linetype=cluster.local,fill=cluster.local))+
  labs(caption = "Local clustering for APE score")+
  geom_smooth(method = "loess", size = 3,se = FALSE,span=2)+
  theme_bw() + 
...
gp4 <- ggplot(data = dat[[1]], aes(x =time2, y =y, color=cluster.global,linetype=cluster.global,fill=cluster.global))+
  labs(caption = "Global clustering for Anxeity score")+
  geom_smooth(method = "loess", size = 3,se = FALSE,span=2)+
  theme_bw() + 
...
gp5 <- ggplot(data = dat[[2]], aes(x =time2, y =y, color=cluster.global,linetype=cluster.global,fill=cluster.global))+
  labs(caption = "Global clustering for Depress score")+
  geom_smooth(method = "loess", size = 3,se = FALSE,span=2)+
  theme_bw() + 
...
gp6 <- ggplot(data = dat[[3]], aes(x =time2, y =y, color=cluster.global,linetype=cluster.global,fill=cluster.global))+
  labs(caption = "Global clustering for APE score")+
  geom_smooth(method = "loess", size = 3,se = FALSE,span=2)+
  theme_bw() + 
...
plot_grid(gp1,gp4, gp2, gp5,gp3,gp6,labels=c("(A)", "(B)" , "(C)", "(D)", "(E)",
                                             "(F)"), ncol = 2,   align = "v")
...
```{r, fig.height=10, fig.width=8, message=F, warning=F, fig.cap="Figure 3. Survival curve for global and local clustering"}
...
gp_survival.anxiety <- ggsurvplot(fit, data = dnew_uq, 
                                 ...
                                 strip.text.x = element_text(size=15),
                                 strip.text.y = element_text(size=15))) +
  labs(caption = "Survival curve for anxiety score local clustering")
#---------------------------------------------------------------------------------------#
...
gp_survival.depress <- ggsurvplot(fit, data = dnew_uq, 
                                  ...
                                  strip.text.x = element_text(size=15),
                                  strip.text.y = element_text(size=15))) +
  labs(caption = "Survival curve for depress score local clustering")
#---------------------------------------------------------------------------------------#
...
gp_survival.aep <- ggsurvplot(fit, data = dnew_uq, 
                              ...
                              strip.text.x = element_text(size=15),
                              strip.text.y = element_text(size=15)))+
  labs(caption = "Survival curve for AEP score local clustering")
